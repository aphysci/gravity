...
%typemap(directorin) const std::vector< std::shared_ptr<gravity::GravityDataProduct> >& dataProducts {
    ...
    for (unsigned int i = 0; i < dataProducts.size(); i++)
    {
        std::shared_ptr<gravity::GravityDataProduct> dataProduct = dataProducts.at(i);
...
namespace gravity {
    ...
    %feature("shadow") GravitySubscriber::subscriptionFilled(const std::vector< std::shared_ptr<gravity::GravityDataProduct> >& dataProducts) %{
	...
	public:
	    ...
	    virtual void subscriptionFilled(const std::vector< std::shared_ptr<gravity::GravityDataProduct> >& dataProducts) = 0;
