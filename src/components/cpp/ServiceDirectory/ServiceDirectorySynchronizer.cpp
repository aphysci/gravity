    ...
    while (true)
    {
        ...
        if (pollItems[0].revents & ZMQ_POLLIN)
        {
            ...
            else if (command == "Update")
            {
                ...
                SpdLog::info(
                    fmt::format("deleted from Synchronizer pollItems: pollItems len = {}", pollItems.size()).c_str());

                // Close SUB socket
                socketToDomainDetailsMap.erase(details->socket);
                SpdLog::trace(
                    fmt::format("deleted from Synchronizer socketToDomainDetailsMap: socketToDomainDetailsMap len = {}",
            ...
            else if (command == "Remove")
            {
                ...
                if (syncMap.find(domain) != syncMap.end())
                {
                    ...
                    SpdLog::info(
                        fmt::format("deleted from Synchronizer pollItems: pollItems len = {}", pollItems.size())
                            .c_str());
        ...
        if (pollItems.size() > 2)
        {
            ...
            while (pollItemIter != pollItems.end())
            {
                if (pollItemIter->revents && ZMQ_POLLIN)
                {
                    ...
                    SpdLog::trace(fmt::format("Response domain:reg time = {}:{}, pollItemIter->socket = {}, "
                                              "socketToDomainDetailsMap size = {}, msg id = {}",
                                              response.getDomain(), response.getRegistrationTime(),
                                              pollItemIter->socket, socketToDomainDetailsMap.size(),
                    ...
                    {
                        SpdLog::debug(fmt::format("Found invalid socket - expecting domain:time {}:{}, but found {}:{} "
                                                  "with GDP id = {} and msg size = {}, ignoring",
                                                  socketToDomainDetailsMap[pollItemIter->socket]->domain,
                                                  socketToDomainDetailsMap[pollItemIter->socket]->registrationTime,
                                                  response.getDomain(), response.getRegistrationTime(),
                                                  response.getDataProductID(), zmq_msg_size(&message))
                                          .c_str());
                    }
                    else if (response.getDataProductID() == "DataProductRegistrationResponse")
                    {
                        ...
                        else
                        {
                            ...
                            SpdLog::info(fmt::format("Received DataProductRegistrationResponse response from "
                                                     "ServiceDirectory for domain: '{}'",
                                                     domain)
                                             .c_str());
                    ...
                    else if (response.getDataProductID() == gravity::constants::DOMAIN_DETAILS_DPID)
                    {
                        ...
                        SpdLog::info(
                            fmt::format("Received update from ServiceDirectory for domain: '{}'", domain).c_str());
                        ...
                        else
                        {
                            ...
                            SpdLog::info(fmt::format("Incremental Update from domain '{}': {} {} named {} at {}",
                                                     domain, change, type, productID, url)
                                             .c_str());
